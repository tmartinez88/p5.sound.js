  ...
  start(successCallback, errorCallback) {
    ...
    window.navigator.mediaDevices
      ...
      .then(function (stream) {
        ...
        self.mediaStream = audioContext.createMediaStreamSource(stream);
  ...
  amp(vol, t) {
    if (t) {
      ...
      this.output.gain.linearRampToValueAtTime(
        vol,
        rampTime + audioContext.currentTime
      );
    } else {
      this.output.gain.cancelScheduledValues(audioContext.currentTime);
      this.output.gain.setValueAtTime(vol, audioContext.currentTime);
    }
  }
